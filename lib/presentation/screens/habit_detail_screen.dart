...
import 'package:loop_habit_tracker/domain/usecases/calculate_habit_score.dart'; // Added
...
class _HabitDetailScreenState extends State<HabitDetailScreen> {
  ...
  void _calculateStatistics() {
    ...
    _habitScore = _calculateHabitScore(
      _currentHabit,
      _repetitions,
      DateTime.now(),
    );

    // Simple success rate: number of repetitions / number of days since creation * 100
  ...
  Future<void> _onDateLongPress(DateTime date) async {
    ...
    final isCompleted = _repetitions.any(
      (rep) => isSameDay(rep.timestamp, dateOnly),
    );
    ...
    if (isCompleted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppLocalizations.of(context)!.cannotSkipCompleted),
        ),
      );
      return;
    }
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          isSkipped
              ? AppLocalizations.of(context)!.unskipDay
              : AppLocalizations.of(context)!.skipDay,
        ),
        content: Text(
          isSkipped
              ? AppLocalizations.of(context)!.confirmUnskip
              : AppLocalizations.of(context)!.confirmSkip,
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
          ...
          TextButton(
            ...
            child: Text(
              isSkipped
                  ? AppLocalizations.of(context)!.unskip
